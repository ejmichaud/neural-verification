...
is the sum of current element and the previous 8 elements
in the sequence.
Sequences are lists of integers in [0, 100) of length 20.
...
if __name__ == "__main__":
    ...
    sequences_x = torch.randint(0, 100, (D, 20), dtype=torch.int16)
    sequences_y = torch.zeros_like(sequences_x, dtype=torch.int16)
    for i in range(20):
        sequences_y[:, i] = sequences_x[:, max(0, i-8):i+1].sum(dim=-1)
