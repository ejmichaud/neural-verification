...
if __name__ == "__main__":
    ...
    D = int(1e6)
    split = 0.8
    sequences_x = torch.randint(0, 16, (D, 10, 1)).type(torch.int32)
    sequences_y = torch.floor(torch.cumsum(sequences_x, dim=1).type(torch.float32)/15).type(torch.int32)
    sequences_y = torch.cat([torch.zeros((D, 1, 1)), sequences_y], dim=1)
