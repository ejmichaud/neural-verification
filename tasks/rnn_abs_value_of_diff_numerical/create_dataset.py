...
is abs(current element minus previous element).
Sequences are lists of integers in [-100, 100) of length 10.

Author: Carl Guo and Eric Michaud

Note from Eric: changed dtype to int16 and use convention that
the answer at the first sequence position is the abs of the
first element (as if the -1th element is 0).
...
if __name__ == "__main__":
    ...
    sequences_y[:, 0] = torch.abs(sequences_x[:, 0]) # first output the abs of first input
    for i in range(1, 10):
