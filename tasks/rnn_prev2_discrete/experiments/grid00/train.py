...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dim', type=int, default=100, help='Output dimension')
    ...
    for step in range(args.steps):
        if step % 100 == 0 or step == args.steps - 1:
            ...
            with torch.no_grad():
                ...
                x = torch.nn.functional.one_hot(x, num_classes=100)
                y_pred, _ = model.forward_sequence(x.to(dtype))
                loss = F.cross_entropy(y_pred.permute(0, 2, 1), y_target)
                test_losses.append(loss.item())
                accuracy = (y_pred.argmax(dim=2) == y_target).float().mean()
