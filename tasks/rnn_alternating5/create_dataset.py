...
sequence of elements of an alternating group of order 5, and the
output is the cumulative group operation over the sequence. The group
has 60 elements. Sequences are of length 10.
...
if __name__ == "__main__":
    ...
    D = int(1e6)
    split = 0.9
    perms = list(sorted(list(set(permutations(tuple(range(5)))))))
#    print(perms)
    cayley_table = torch.Tensor([[perms.index(tuple(perms[i][k] for k in perms[j])) for j in range(120)] for i in range(120)]).type(torch.int64)
#    print(cayley_table)
    alternating_elements = torch.Tensor([i for i in range(120) if sum([int(perms[i][x]>perms[i][y]) for y in range(5) for x in range(y)]) % 2 == 0]).type(torch.int64)
    nonalternating_elements = torch.Tensor([i for i in range(120) if sum([int(perms[i][x]>perms[i][y]) for y in range(5) for x in range(y)]) % 2 != 0]).type(torch.int64)
