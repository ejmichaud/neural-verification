...
class Circle:
    ...
    def loosen_forward(self, weights, token):
        return self.affine_forward(weights["hmlp.mlp.0.weight"].numpy(), weights["hmlp.mlp.0.bias"].numpy()+weights["in2hidden.weight"].numpy()[:,token]).loosen_tanh_forward()
    def tighten_backward(self, weights, token):
        return self.tighten_tanh_backward().affine_forward(weights["hmlp.mlp.0.weight"].numpy(), weights["hmlp.mlp.0.bias"].numpy()+weights["in2hidden.weight"].numpy()[:,token])
...
class NFA:
    def __init__(self, n_tokens, weights):
        ...
        self.n_classes = self.weights["ymlp.mlp.0.bias"].numpy().shape[0]
        # The variables below encode the shapes of the convex regions of hidden space where one class is chosen over another
        out_weights = self.weights["ymlp.mlp.0.weight"].numpy()
        out_biases = self.weights["ymlp.mlp.0.bias"].numpy()
