...
already trained. Well, it doesn't do the training itself,
...
if __name__ == '__main__':
    ...
    # hammered_networks_dir = "/om2/user/ericjm/neural-verification/hammered_models2"
    first_paper_dir = "/om2/user/ericjm/neural-verification/first_paper"
    tasks_dir = "/om2/user/ericjm/neural-verification/tasks"
    train_l1_script = "/om2/user/ericjm/neural-verification/scripts/train_l1.py"
    training_steps = int(3e5)
    EPSILON = 5e-5
    # choose new directory to save in based on epsilon
    regularized_networks_dir = f"/om2/user/ericjm/neural-verification/regularized_models_long{EPSILON}"

    # create if doesn't exist
    os.makedirs(regularized_networks_dir, exist_ok=True)
    ...
    tasks = [d for d in os.listdir(first_paper_dir) if os.path.isdir(os.path.join(first_paper_dir, d))]
    print(f"Found {len(tasks)} tasks that have been trained.")
    for task in tasks:

        # create a directory for the task
        os.makedirs(os.path.join(regularized_networks_dir, task), exist_ok=True)
        ...
        args += f"--model_state_dict {os.path.join(first_paper_dir, task, 'model_perfect.pt')} "
        args += f"--save_dir {os.path.join(regularized_networks_dir, task)} "
        args += f"--steps {training_steps} "
        args += f"--epsilon {EPSILON} "
        
        # submit a job to train the network
        slurm_script = SLURM_TEMPLATE.format(
            task=task, # this is just needed for the slurm job name
            path=os.path.join(regularized_networks_dir, task),
            args=args,
            training_script=train_l1_script
        )
