...
rotation = torch.eye(hidden_dim, requires_grad=True)

optimizer = torch.optim.AdamW([rotation], lr=0.001)

for step in range(train_steps):
    Q, R = torch.linalg.qr(rotation)
    true_rotation = Q
    S = torch.linalg.svdvals(rotation)
    loss = torch.mean(torch.abs(torch.matmul(hidden_states, true_rotation))) + torch.mean(torch.abs(S-1))
    optimizer.zero_grad()
    loss.backward()
    optimizer.step()
U, S, Vh = torch.linalg.svd(rotation)
true_rotation = torch.matmul(U, Vh)

with torch.no_grad():
    rnn.hmlp.mlp[-1].weight = nn.Parameter(torch.matmul(true_rotation.transpose(0, 1), rnn.hmlp.mlp[-1].weight))
    rnn.hmlp.mlp[-1].bias = nn.Parameter(torch.matmul(true_rotation.transpose(0, 1), rnn.hmlp.mlp[-1].bias))
    rnn.hmlp.mlp[0].weight = nn.Parameter(torch.cat([torch.matmul(rnn.hmlp.mlp[0].weight[:,:hidden_dim], true_rotation), rnn.hmlp.mlp[0].weight[:,hidden_dim:]], dim=1))
    rnn.ymlp.mlp[0].weight = nn.Parameter(torch.matmul(rnn.ymlp.mlp[0].weight, true_rotation))

# Put the new weights and biases into modified_model.pt
torch.save(rnn.state_dict(), modified_fname)

print('Rotated RNN hidden space using matrix: ' + str(true_rotation))
