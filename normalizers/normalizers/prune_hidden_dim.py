...
with torch.no_grad():
    ...
    hidden_writes = rnn.hmlp.mlp[-1].weight.numpy()
    hidden_hmlp_reads = rnn.hmlp.mlp[0].weight.numpy()[:,:hidden_dim]
    hidden_ymlp_reads = rnn.ymlp.mlp[0].weight.numpy()
    keep_dimensions = np.sqrt(np.sum(hidden_writes**2, axis=1))*np.sqrt(np.sum(hidden_hmlp_reads**2, axis=0) + np.sum(hidden_ymlp_reads**2, axis=0)) > prune_threshold
    discard_dimensions = np.logical_not(keep_dimensions)
    n_keep_dimensions = np.sum(keep_dimensions.astype(np.int32))
    ...
    discard_activations = activation()(torch.from_numpy(hmlp_biases[-1][discard_dimensions]))

    hmlp_weights[-1] = hmlp_weights[-1][keep_dimensions,:]
    hmlp_biases[-1] = hmlp_biases[-1][keep_dimensions]
    hmlp_biases[0] = hmlp_biases[0] + hmlp_weights[0][:,np.concatenate([discard_dimensions, np.zeros([input_dim]).astype(bool)], axis=0)].dot(discard_activations)
    hmlp_weights[0] = hmlp_weights[0][:,np.concatenate([keep_dimensions, np.ones([input_dim]).astype(bool)], axis=0)]
    ymlp_biases[0] = ymlp_biases[0] + ymlp_weights[0][:,discard_dimensions].dot(discard_activations)
    ymlp_weights[0] = ymlp_weights[0][:,keep_dimensions]
...
new_config = GeneralRNNConfig(input_dim, output_dim, n_keep_dimensions, hidden_mlp_depth, hidden_mlp_width, output_mlp_depth, output_mlp_width, activation)
...
print('Original hidden dimension: ' + str(hidden_dim) + '. New hidden dimension: ' + str(n_keep_dimensions))
